...
upload_cpu_usage(sse_int in_timer_id, sse_pointer in_user_data)
{
    ...
    SensingData cpu_usage;
    UserContext* ctx = (UserContext*)in_user_data;
    sse_char urn[512];
    
    err = get_sensor_data(&cpu_usage);
    ...
    moat_object_add_float_value(object, "temperature", cpu_usage.temperature, sse_false);
    moat_object_add_float_value(object, "humidity", cpu_usage.humidity, sse_false);
